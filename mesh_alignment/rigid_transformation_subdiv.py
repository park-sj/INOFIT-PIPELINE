...
for patient in patients:
    ...
    if pathLoad[-3:] == 'off':
        ...
        vertex_source, faces_source = read_off("template_right.off")
        y = _load_yaml('template_subdiv.yaml')
        ...
        vertex_result = reg.transform_point_cloud(vertex_source)
        save_off(vertex_result, faces_source, osp.join(patients_dir, patient[:-4] + '_template_right.off'))

        vertex_source, faces_source = read_off("template_left.off")
        y = _load_yaml('template_subdiv.yaml')
        fish_source = np.zeros((19,3)).astype(np.float64)
        landmark = y['landmark']
        for i in range(19):
        ...
        reg.register(callback)
        vertex_result = reg.transform_point_cloud(vertex_source)
        save_off(vertex_result, faces_source, osp.join(patients_dir, patient[:-4] + '_template_left.off'))
