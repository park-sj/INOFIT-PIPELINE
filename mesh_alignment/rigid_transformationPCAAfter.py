...
for patient in patients:
    ...
    patientOFF = 'targetToPCA_E_final.off'
    pathLoadOFF = osp.join(patients_dir, patientOFF)
    #print('pathLoad' + str(pathLoad) + ' ///  patientOFF ' + patientOFF + ' ///   pathLoadOFF ' + pathLoadOFF)
    vertex_target, faces_target = read_off(pathLoadOFF)

    if pathLoad[-9:] == 'final.off':
        ...
        vertex_source, faces_source = read_off("../io/landmark/target.off")
        fish_source = np.zeros((19,3)).astype(np.float64)
        with open('../io/landmark/target.txt') as t:
            for i in range(19):
                l = t.readline()
                fish_source[i] = np.array(l.split(' '))
        ...
        save_off(vertex_result, faces_target, osp.join(patients_dir, 'PCAResult.off'))
#patient[:-4] + '_target.off'))
