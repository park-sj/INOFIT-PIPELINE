...
for patient in patients:
    ...
    patientOFF = 'landmarkTemplate.off'
    pathLoadOFF = osp.join(patients_dir, patientOFF)
    vertex_target, faces_target = read_off(pathLoadOFF)

    if pathLoad[-3:] == 'off':

        vertex_source, faces_source = read_off("./PCATest/target.off")
        fig = plt.figure()
        ax = fig.add_subplot(111, projection='3d')
        callback = partial(visualize, ax=ax)

        reg = RigidRegistration(**{'X': vertex_source, 'Y': vertex_target})
        reg.register(callback)
        vertex_result = reg.transform_point_cloud(vertex_target)
        #save_off(vertex_result, faces_target, osp.join(patients_dir, 'PCAFinal.off'))
        save_off(vertex_result, faces_target, './PCATest/PCAFinal.off')
