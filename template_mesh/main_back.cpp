...
{   
  ...
  /* size_t stiffness_parameters = 6;
  double stiffness[stiffness_parameters] = {600.0, 300.0, 200.0, 100.0, 80.0,50.0}; */
  size_t stiffness_parameters = 3;
  double stiffness[stiffness_parameters] = {600.0, 300.0, 200.0};
  Eigen::MatrixXd X;
  Eigen::MatrixXd Xpre;
  ...
  for( int k =0; k < stiffness_parameters; k++)
  {
    ...
    while( iteration < inner_loop_count && epsilon > 0.1)
    {
      ...
      Eigen::SparseMatrix<double, Eigen::RowMajor> somematrix;
    	...
    	create_correspondence_matrix_with_landmarks<Point, double, Traits>(targetMeshTree, templateMesh, D,U, landmarks);
    	/* Concatenate M and D */
