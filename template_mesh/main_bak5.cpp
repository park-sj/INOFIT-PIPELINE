...
{   
  ...
  typedef Eigen::SparseMatrix<double, Eigen::RowMajor> SparseMatrixType; 
	...
	Eigen::MatrixXd col = AtB.col(0);

	double *col_data = col.data();
	...
	csr_matrix<int, double> AtA_cuda;
	AtA_cuda.num_rows =(int)AtA.rows();
	AtA_cuda.num_cols =(int)AtA.cols();
	AtA_cuda.Ax = values;
	AtA_cuda.num_nonzeros = values_length;
	AtA_cuda.Ap = rows;
	AtA_cuda.Aj = cols;

	vec<int, double> x = new_vec<int,double>(col_length);
	std::cout<<"solving column "<<std::endl;
	cg<int, double>(AtA_cuda, x, b);

	std::cout<<x.val[1]<<std::endl;
	std::cout<<x.val[2]<<std::endl;
...
template<typename T, typename S>
void allocate_cuda_matrix(Eigen::SparseMatrix<S>& A_in, csr_matrix<T, S>& A_out)
{
    Eigen::SparseMatrix<double>::StorageIndex *outerStarts = A_in.outerIndexPtr();
    Eigen::SparseMatrix<double>::StorageIndex *innerIndices = A_in.innerIndexPtr();
    Eigen::SparseMatrix<double>::Scalar *values = A_in.valuePtr();
    ...
    memcpy(csr_row_pointers,outerStarts, outerStartsLength);
    memcpy(csr_col_pointers,innerIndices,valuesLength);
    memcpy(csr_values, values,valuesLength); 

    /* add one to index to convert to one based indexing */
    for (int i = 0; i <=A_in.outerSize(); i++)
    {
        csr_row_pointers[i]+=1;
    }
    for (int i = 0; i <=A_in.nonZeros(); i++)
    {
        csr_col_pointers[i]+=1;
    }
