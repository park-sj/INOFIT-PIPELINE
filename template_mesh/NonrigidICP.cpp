...
template<typename T,typename S, typename R>
int create_correspondence_matrix2(CGAL::AABB_tree<R>& tree, CGAL::Surface_mesh<T>& mesh, CGAL::Surface_mesh<T>& targetMesh, std::vector<std::tuple<size_t, size_t>> landmarks, Eigen::SparseMatrix<S>& D, Eigen::SparseMatrix<S>& U)
{
   /* Iterate through template mesh and find closest poin on target mesh*/
    ...
    std::set<size_t> landmark_indices;
    std::set< std::tuple<size_t, size_t> >::iterator lItr = landmarks.begin();
    for(lItr; lItr < landmarks.begin(); lItr++)
    {
	std::tuple<size_t, size_t> landmark = *lItr;
	size_t templateIdx = std::get<0>(landmark);
	size_t targetIdx = std::get<1>(landmark);
	landmark_indices.push_back(templateIdx); 
    }
	...
	if ( landmark_indices.find(v_idx) != landmark_indices.end() )
	{
