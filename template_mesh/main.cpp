...
template<typename T,typename S, typename R>
int create_correspondence_matrix_with_landmarks(CGAL::AABB_tree<R>& tree, CGAL::Surface_mesh<T>& mesh, Eigen::SparseMatrix<S, Eigen::RowMajor>& D, Eigen::SparseMatrix<S, Eigen::RowMajor>& U,
     std::map<size_t, Point>& landmarks, int fill_flag);

template <class IndexType, class ValueType>
int cg(csr_matrix<IndexType, ValueType> A, vec<IndexType, ValueType> x,
        vec<IndexType, ValueType> b)
{
  int size_b = A.num_rows;
  int size_row_ptr = A.num_rows+1;
  int size_col_idx = A.num_nonzeros;
  int error = gpucg_solve_(A.Ap, &size_row_ptr, A.Aj, &size_col_idx, A.Ax, &size_col_idx,
               b.val, &size_b, x.val);
  return error;
}
...
{   
    ...
    /* create two maps, zero based indexing
       create a set of target indices 

      int : indices -> vertex indices 
      target_indices = {   }

      iterate through template map, get only if the indices in target indices set
    */

    std::map<size_t, size_t> template_landmarks_inv;
    std::map<size_t, size_t> target_landmarks; 
    std::map<size_t, size_t> landmark_correspondences;
    std::map<size_t, Point> landmarks; 
    /* fill flag, fill missing points with template if set to one, default zero*/
    int fill_flag = 0;

 
    if (argc >= 5){
      YAML::Node targetYAML = YAML::LoadFile(argv[4]);
      if (targetYAML["landmark"])
        {
          std::cout << "Target mesh YAML has been successfully loaded!" << std::endl;
          YAML::const_iterator it = targetYAML["landmark"].begin();
          for (YAML::const_iterator it = targetYAML["landmark"].begin(); it != targetYAML["landmark"].end(); it++)
            {
                size_t index = it->first.as<size_t>();
                size_t vertex_index = it->second.as<size_t>();
                target_landmarks[index] = vertex_index; 
            }
        }
      else
        {
          std::cout<< "Target mesh YAML not loaded"<<std::endl;
        }
    }

    if (argc >= 6){
      YAML::Node targetYAML = YAML::LoadFile(argv[5]);
      if (targetYAML["landmark"])
        {
          std::cout << "Template mesh YAML has been successfully loaded!" << std::endl;
          YAML::const_iterator it = targetYAML["landmark"].begin();
          for (YAML::const_iterator it = targetYAML["landmark"].begin(); it != targetYAML["landmark"].end(); it++)
            {
                size_t index = it->first.as<size_t>();
                size_t vertex_index = it->second.as<size_t>();
                template_landmarks_inv[vertex_index] = index; 
            }
        }
    }

    if (argc == 7){
       // std::string arg7str(argv[7]);
       std::istringstream(argv[6]) >> fill_flag;
       // fill_flag = atoi(argv[7]
       // fill_flag = std::stoi( arg7str );
       std::cout<<fill_flag<<std::endl;
    }

    std::map<size_t, size_t>::iterator itr;
    for( itr = template_landmarks_inv.begin(); itr != template_landmarks_inv.end(); itr++)
    {
        size_t vertex_idx = itr->first;
        size_t idx = itr->second;

        if ( target_landmarks.find(idx) == target_landmarks.end() ) {
          std::cout<<"Not found"<<std::endl;
        } else {
          std::cout<<"idx :"<<idx<<std::endl;
          size_t target_vertex_idx = target_landmarks[idx];
          std::cout<<target_vertex_idx<<std::endl;
          landmark_correspondences[vertex_idx] = target_vertex_idx;
          Mesh::Vertex_index v_idx(target_vertex_idx);
          Point p = targetMesh.point(v_idx);
          std::cout<<p<<std::endl;
          landmarks[vertex_idx]=p; 
        }
    ...
    /* create a map from temlate indices to target indices */


    if (!templateInput)
    {
      std::cerr << "Cannot open template file " << std::endl;
      return EXIT_FAILURE;
    }

    std::vector< std::tuple<size_t, size_t> > directedEdges;

    std::cout<<"Template mesh"<<std::endl;
    std::cout<<"Vertices: "<<templateMesh.number_of_vertices()<<std::endl;
    std::cout<<"Faces: "<<templateMesh.number_of_faces()<<std::endl;
    ...
    size_t template_vertex_count = templateMesh.number_of_vertices();
      ...
      { 
          std::cout<<"failed creating oct tree"<<std::endl;
	  std::cout<<e<<std::endl;
	  return -1;
      }    

  /* concludes preprocessing */
  size_t stiffness_parameters = 4;
  double stiffness[stiffness_parameters] = {2000.0, 300.0,2000.0, 200.0};
  Eigen::MatrixXd X;
  Eigen::MatrixXd Xpre;
  int error = 0;
  /* outer loop, stiffness parameter control*/
  for( int k =0; k < stiffness_parameters; k++)
  {
    Eigen::SparseMatrix<double, Eigen::RowMajor> M;
    double alpha = stiffness[k];
    create_incidence_matrix<Point, double>(templateMesh, M, alpha);

    int inner_loop_count = 10;
    double epsilon = 10000;
    size_t iteration = 0; 
    std::cout<<"stifness :"<<alpha<<std::endl;
    while( iteration < inner_loop_count && epsilon > 0.1)
    {
      /* preallocate D,U */
      Eigen::SparseMatrix<double, Eigen::RowMajor> somematrix;
    	...
    	create_correspondence_matrix_with_landmarks<Point, double, Traits>(targetMeshTree, templateMesh, D,U, landmarks, fill_flag);
    	...
    	int error1 = cg<int, double>(AtA_cuda_1, x_1, b_1);
        ...
        int error2 = cg<int,double>(AtA_cuda_2, x_2, b_2);
        ...
        int error3 = cg<int,double>(AtA_cuda_3, x_3, b_3);
        
        if (error1 != 0 or error2 != 0 or error3 != 0)
          error = -1;

      /* create matrix X from x_1,x_2,x_3 */
      Eigen::SparseMatrix<double> X_sparse(col_length,3);
     //Eigen::MatrixXd X;
      std::vector< Eigen::Triplet<double> > XtripletList;
      for( int i = 0; i < col_length; i++)
      {    
        XtripletList.push_back(Eigen::Triplet<double>(i, 0, x_1.val[i] ));
        XtripletList.push_back(Eigen::Triplet<double>(i, 1, x_2.val[i] ));
        XtripletList.push_back(Eigen::Triplet<double>(i, 2, x_3.val[i] ));
      }

      X_sparse.setFromTriplets(XtripletList.begin(), XtripletList.end()); 
      

      X = Eigen::MatrixXd(X_sparse);
      ...
      Eigen::MatrixXd convergence = Xpre - X;  
      Xpre = X;
      epsilon = convergence.norm();
      iteration++;

      std::cout<<"|| X_j-1 - X_j || :"<<convergence.norm()<<std::endl;
      if (error != 0)
        break;   
  	}
  }
...
int create_correspondence_matrix(CGAL::AABB_tree<R>& tree, CGAL::Surface_mesh<T>& mesh, Eigen::SparseMatrix<S, Eigen::RowMajor>& D, Eigen::SparseMatrix<S, Eigen::RowMajor>& U)
{
    ...
    */
    ...
    for( v_itr = v_begin; v_itr != v_end; v_itr++ )
    {
        ...
        K::FT distance_to_closest = CGAL::squared_distance(p,closest); 
        double d = distance_to_closest;
       // std::cout<<"distance :"<<d<<std::endl;
        ...
        /* if too far, don't bother using closest */
      ...
      std::map<size_t, Point>& landmarks, int fill_flag)
{
    ...
    */
    ...
    for( v_itr = v_begin; v_itr != v_end; v_itr++ )
    {
        ...
        /* if v_idx is one of the landmark indices, process separately */
        /*
        if ( landmarks.find(v_idx) == landmarks.end() ) {
          Point_and_primitive_id pp = tree.closest_point_and_primitive(p);
          Point closest = pp.first;
          if ( fill_flag == 1 ){
              K::FT distance_to_closest = CGAL::squared_distance(p,closest); 
          ...
          correspondence = closest;
        } else {
          correspondence = landmarks[v_idx];
        } */
        ...
        double u1 = correspondence[0];
        double u2 = correspondence[1];
        double u3 = correspondence[2];
