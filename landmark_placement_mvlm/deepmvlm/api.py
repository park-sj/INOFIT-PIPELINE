...
class DeepMVLM:
    def __init__(self, config):
        self.config = config
        if self.config['name'] == 'MVLMModel_CTMASK':
    ...
    def _get_device_and_load_model(self):
        ...
        if model_name == "MVLMModel_DTU3D":
            ...
            if image_channels == "RGB":
                ...
                print('No model trained for ', model_name, ' with channels ', image_channels)
                return None, None
        elif model_name == 'MVLMModel_CTMASK':
            check_point_name = 'saved/trained/model_best.pth'
        else:
    ...
    def predict_one_file(self, file_name):
        ...
        avg_error = u3d.compute_all_landmarks_from_view_lines()
        u3d.project_landmarks_to_surface(file_name)

        return u3d.landmarks, avg_error
    ...
    @staticmethod
    def write_landmarks_as_obj(landmarks, file_name):
        Utils3D.write_landmarks_as_obj_external(landmarks, file_name)
