...
def process_one_file(config, file_name):
    ...
    name_lm_vtk = os.path.splitext(file_name)[0] + '.vtk'
    name_lm_obj = os.path.splitext(file_name)[0] + '.obj'
    dm = deepmvlm.DeepMVLM(config)
    landmarks, avg_error = dm.predict_one_file(file_name)
    # dm.write_landmarks_as_vtk_points(landmarks, name_lm_vtk)
    dm.write_landmarks_as_obj(landmarks, name_lm_obj)
...
def process_file_list(config, file_name):
    ...
    for file_name in names:
        print('Processing ', file_name)
        name_lm_txt = os.path.splitext(file_name)[0] + '.txt'
...
def main(config):
    ...
    if name.lower().endswith(('.obj', '.wrl', '.vtk', '.ply', '.stl', '.off')) and os.path.isfile(name):
