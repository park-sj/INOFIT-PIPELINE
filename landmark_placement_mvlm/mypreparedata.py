...
def process_file_ctmask(config, file_name, output_dir):
    def apply_pre_transformation(pd, lms, config):
        translation = [0, 0, 0]
        if config['pre-align']['align_center_of_mass']:
        ...
        rx = config['pre-align']['rot_x']
        ry = config['pre-align']['rot_y']
        rz = config['pre-align']['rot_z']
        s = config['pre-align']['scale']
        ...
        if config['pre-align']['write_pre_aligned']:
            name_out = str(config.temp_dir / ('pre_transform_mesh.vtk'))
            writer = vtk.vtkPolyDataWriter()
            writer.SetInputData(trans.GetOutput())
            writer.SetFileName(name_out)
            writer.Write()
    
        return trans.GetOutput(), trans_lm.GetOutput()
    
    bu_3dfe_dir = config['preparedata']['raw_data_dir']
    base_name = os.path.basename(file_name)
    name_pd = bu_3dfe_dir + file_name + '.obj'
    name_lm = bu_3dfe_dir + file_name + '.txt'
    print(f"name_pd : {name_pd}, name_lm : {name_lm}")
    ...
    if pd is None or pd.GetNumberOfPoints() < 1:
        ...
        return None
    print(config['pre-align'])
    pd, lms = apply_pre_transformation(pd, lms, config)
    ...
    trans.SetTransform(t)
    trans.Update()
    print(trans.GetOutput())

    trans_lm = vtk.vtkTransformPolyDataFilter()
    trans_lm.SetInputData(lms)
    trans_lm.SetTransform(t)
    trans_lm.Update()
    print(trans_lm.GetOutput())
    ...
    del writer_png_2, writer_png, ren_win, actor_geometry, actor_text, mapper, w2if, t, trans, obj_in
    # del texture_image
    del lms, trans_lm
...
def split_CTMASK_into_train_and_test(base_file_names, output_dir):
    train_file = output_dir + "dataset_train.txt"
    test_file = output_dir + "dataset_test.txt"
    # F0001-F0046 + M0001-M0034 are used for training
    # F0047-F0056 + M0035-M0054 are used for testing
    ...
    for i, name in enumerate(base_file_names):
        if i < 0.8 * len(base_file_names):
            new_set.append(name)
            f1.write(name + '\n')
        else:
            f2.write(name + '\n')
...
def prepare_ctmask_data(config):
    print('Preparing CTMASK data')
    file_id_list = config['preparedata']['raw_data_dir'] + 'CTMASK_base_filelist_noproblems.txt'
    output_dir = config['preparedata']['processed_data_dir']
    image_out_dir = output_dir + 'images/'
    lm_out_dir = output_dir + '2D LM/'
    ...
    base_file_names = split_CTMASK_into_train_and_test(base_file_names, output_dir)

    print('Processing ', len(base_file_names), ' file ids for training')

    for base_name in base_file_names:
        print('Processing ', base_name)
        process_file_ctmask(config, base_name, output_dir)

def main(config):
    model_data_name = config['name']
    if model_data_name.find('BU_3DFE') > -1:
        prepare_bu_3dfe_data(config)
    elif model_data_name.find('CTMASK') > -1:
        prepare_ctmask_data(config)
    else:
        print('Cannot recognize data name. Currently supported - BU_3DFE, CTMASK')
