...
class Utils3D:
    ...
    def multi_read_surface(file_name):
        ...
        if file_extension == ".obj":
            ...
            with open(file_name, 'r') as file:
                line = file.readlines()
                if line[0][:3] == "OFF" or line[0][:4] == "COFF":
                    line = line[1:]
                nv, nf, _ = map(int, line[0].strip("\n").split(" "))
                line = line[1:]
                points = vtk.vtkPoints()
                for v in range(nv):
                    x, y, z = map(np.double, line[v].strip().split(" "))
                    points.InsertNextPoint(x, y, z)
                pd.SetPoints(points)
                triangles = vtk.vtkCellArray()
                line = line[nv:]
                for f in range(nf):
                    _, x, y, z = map(np.int, line[f].strip().split(" "))
    ...
    @staticmethod
    def write_landmarks_as_obj_external(landmarks, file_name):
        f = open(file_name, 'w')
        ...
        for lm in landmarks:
            ...
            py = lm[1]
            pz = lm[2]
            out_str = 'v ' + str(px) + ' ' + str(py) + ' ' + str(pz) + '\n'
            f.write(out_str)
